 ...
 * prototype property: "on<Eventname>" (onDragstart, onMouseup,..), which will be called if the event occurs.
 * Provided Events:
...
function Stage(canvasId){
	...
	this.theCanvas.parentNode.addEventListener('mousemove', function(e){
		// get Mouse Coords
		var mousePos = {x:e.layerX, y:e.layerY};
		// if we are draggin some object, update position
		if(stage.dragging){
...
Stage.prototype._checkTransparency = function(mousePos){
	...
	var x;
	var y;
	x=this.currentMouseObj.x;
	y=this.currentMouseObj.y;
	...
	//console.log(this.currentMouseObj.width * this.currentMouseObj.height);
	//console.log(imageData.data[stelle]);
...
/**
 * Render the stage. The objects to be rendered had to be added 
 * to the stage by "addToStage" 
 *
 */
